...
const PetSizeSelector = ({ animal = "dog", weight, onWeightSelect }) => {
  ...
  // Filtra los tamaños dependiendo del animal
  const filteredSizes =
    animal === "cat"
      ? sizes.filter((size) => ["xs", "s"].includes(size.key))
      : sizes;
  ...
  if (weight) {
    const size = sizes.find((s) => s.range == weight);
    return size ? (
      renderSizeOption(size)
    ) : (
      <Typography>No matching size found</Typography>
    );
  }

  return (
    <ScrollContainer>{filteredSizes.map(renderSizeOption)}</ScrollContainer>
  );
};
