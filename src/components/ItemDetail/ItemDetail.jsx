...
const ItemDetail = ({ item }) => {
  const { id, name, dimensions, corner, coverPhoto } = item;
  ...
  const handleSave = () => {
    ...
    const updatedSavedItemIds = actualSavedItemsIds.includes(id)
      ? actualSavedItemsIds.filter((itemId) => itemId !== id)
      : [...actualSavedItemsIds, id];
  ...
  return (
    <>
      <Card sx={{ maxWidth: 345, margin: "auto" }}>
        ...
        <CardContent>
          ...
          <Typography
            ...
            gutterBottom
          >
            Dimensions: {dimensions}. {corner ? "Corner lot" : "Adjacent lots"}
          </Typography>
        </CardContent>
        <CardActions sx={{ justifyContent: "space-evenly" }}>
          <Button
            ...
            onClick={handleSave}
          >
            {savedItemIds.includes(id) ? "GUARDADO" : "GUARDAR"}
          </Button>
          <Button
            variant="contained"
            endIcon={<CottageIcon />}
      ...
      <ItemPopup open={popupOpen} onClose={handleClosePopup} itemId={id} />
    </>
  );
};
